...
description: "Register the companion with Claude Code, Codex, OpenCode, Cursor, Antigravity or Muse Code over stdio."
---
...
<Tabs>
  ...
  <Tab title="Muse Code">
    Add to `~/.config/muse/settings.json` (or `$XDG_CONFIG_HOME/muse/settings.json`), keeping any existing servers:
    ```json
    {
      "schema_version": 1,
      "mcpServers": {
    ...
    ```
    Restart Muse Code and check that Browspark is listed among its MCP servers. `mode: "optional"` means a companion failure does not stop Muse Code from launching. The setup script writes this entry for you.
  </Tab>
</Tabs>
