...
```mermaid
flowchart LR
  A[MCP client<br/>Claude Code, Codex, Cursor…] -- "MCP over stdio or HTTP" --> C[Companion<br/>bunx browspark-mcp]
  C -- "WebSocket 127.0.0.1:9223<br/>pairing token" --> E[Chrome extension<br/>service worker + dashboard]
  E -- "chrome.debugger" --> T[Shared tabs in your Chrome]
  C -- "direct CDP" --> D[Developer-mode Chrome<br/>own profile]
```
