...
| A supported browser | Chrome, Brave, Edge or Chromium for extension mode. Developer mode launches Chrome or Firefox. Use `BROWSPARK_CHROME` or `BROWSPARK_FIREFOX` to select an executable. |
...
The downloaded extension is for Chromium. For Firefox, register the companion and use [a Firefox developer session](/reference/firefox); skip the extension-loading step.
...
| `BROWSPARK_CHROME` | auto-detected | Chromium executable for developer mode. |
| `BROWSPARK_FIREFOX` | auto-detected | Firefox executable for developer mode; overridden by `firefoxPath` on launch. |
| `BROWSPARK_PROFILE` | `~/.browspark/profile` | Profile directory of the default Chrome developer context. |
| `BROWSPARK_PROFILES` | `~/.browspark/profiles` | Parent directory for named Chrome contexts; Firefox profiles live in its `.firefox/<context>` subdirectory. |
...
The extension supports Chromium-based browsers. Firefox support uses the companion directly through WebDriver BiDi and does not require an extension. See [Firefox setup and coverage](/reference/firefox).
