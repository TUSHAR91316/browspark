...
| An MCP client | Claude Code, Codex, OpenCode, Cursor, Kilo, Antigravity, Gemini, or anything that speaks MCP over stdio or Streamable HTTP. |
...
Started by an MCP client it speaks MCP over stdio. On every start it also opens `127.0.0.1:9223` for the extension's WebSocket bridge, the [HTTP MCP endpoint](/connect/http-and-gemini) and the [live view](/concepts/developer-browser#live-view). If that port is already taken by another companion, the new process becomes a relay to the existing one instead of failing. See [Multiple agents](/connect/multiple-agents).
