...
In extension mode, the agent reaches exactly the tabs you share, plus tabs it opened itself in the selected browser. `chrome://` pages, extension pages and the Web Store are never automatable. Each Chrome or Brave profile enforces its own sharing. Sharing a tab in one browser grants no access in another.
