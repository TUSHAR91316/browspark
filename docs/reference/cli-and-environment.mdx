...
| `BROWSPARK_FIREFOX` | auto-detected | Firefox executable for `browser_session` with `browser: "firefox"`; `firefoxPath` overrides it. |
| `BROWSPARK_PROFILE` | `~/.browspark/profile` | Profile directory of the `default` Chrome developer context. |
| `BROWSPARK_PROFILES` | `~/.browspark/profiles` | Parent directory of named Chrome contexts; Firefox profiles live in its `.firefox/<context>` subdirectory. |
