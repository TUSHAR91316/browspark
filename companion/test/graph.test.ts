...
test('graph subscriptions publish current metadata without page data or duplicate updates', async () => {
  const connections: BridgeConnection[] = [
    { id: 'chrome', browser: 'Chrome', policy: { disabled: [], graph: true }, tabs: [{ id: 1, url: 'https://private.example', title: 'Private title', windowId: 1, shared: true, attached: false }] },
  ...
  const bridge = Object.assign(new EventEmitter(), {
    ...
    request: async (method: string, graph: ConnectionGraph, _timeout: number, id: string) => { assert.equal(method, 'graph.state'); received.push({ id, graph }); },
  }) as unknown as Bridge;
  const sessions = { runningDevs: () => [{ name: 'dev-firefox', browserName: 'firefox', version: 'Firefox 153', listTabs: () => [{ url: 'https://secret.example' }] }] } as unknown as Sessions;
